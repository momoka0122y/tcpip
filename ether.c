...
ether_transmit_helper(struct net_device *dev, uint16_t type, const uint8_t *data, size_t len, const void *dst, ether_transmit_func_t callback)
{
    ...
    //     Ethernetフレームの生成
    hdr = (struct ether_hdr *)frame;
    memcpy(hdr->dst, dst, ETHER_ADDR_LEN);    // ・ヘッダの各フィールドに値を設定
    memcpy(hdr->src, dev->addr, ETHER_ADDR_LEN);
    hdr->type = hton16(type);
    memcpy(hdr+1, data, len);    // ・ヘッダの直後にデータをコピー
    if (len < ETHER_PAYLOAD_SIZE_MIN) {  //最小サイズに満たない場合はパディングを挿入してサイズを調整
    ...
    // ※ 実際の書込み処理は ether_input_helper() を呼び出したドライバの関数の中で行われる
...
ether_input_helper(struct net_device *dev, ether_input_func_t callback)
{
    ...
    // ※実際の読み込み処理は ether_input_helper() を呼び出したドライバの関数の中で行われ、ether_input_helper() は結果だけ受け取る
